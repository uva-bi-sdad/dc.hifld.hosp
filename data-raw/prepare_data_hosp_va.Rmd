...
provider <- sf::st_read(
  ...
  query= "
    SELECT *
  FROM dc_health_behavior_diet.us_pl_hifld_2020_hospitals_geoblk
  WHERE state='VA'  AND status='OPEN'  AND type != 'PSYCHIATRIC' AND type != 'MILITARY' AND beds > 0 "  
)

#change names lat lon
names(provider)[names(provider)=='latitude'] <- 'lat'
names(provider)[names(provider)=='longitude'] <- 'lon'
names(provider)[names(provider)=='id'] <- 'identif'
...
# save new data
```{r}
write.csv(st_drop_geometry( provider[, c("ID", "address", "lat", "lon", "doctors", "beds")] ), "provider.csv", row.names = FALSE)
